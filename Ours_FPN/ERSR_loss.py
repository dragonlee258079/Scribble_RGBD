...
class ERSR(torch.nn.Module):
    ...
    """
    def forward(
            self, y_hat_softmax, edge_mask, kernels_desc, kernels_radius, sample, height_input, width_input,
        ...
        y_hat_unfolded = y_hat_unfolded[:, :, kernels_radius, kernels_radius, :, :].view(N, C, 1, 1, height_pred, width_pred) - y_hat_unfolded
        y_hat_unfolded = torch.norm(y_hat_unfolded, p=2, dim=1, keepdim=True)

        loss = (kernels * y_hat_unfolded).view(N, 1, (kernels_radius * 2 + 1) ** 2, height_pred, width_pred).sum(dim=2)

        edge_mask[edge_mask < 0.8] = 0.
        edge_mask[edge_mask >= 0.8] = 1.
        loss = edge_mask * loss

        ratio = height_pred * width_pred / (torch.sum(edge_mask.view(N, 1, -1), dim=2) + 1e-8)
        loss = ratio * torch.mean(loss.view(N, 1, -1), dim=2)
        ...
        for i, desc in enumerate(kernels_desc):
            ...
            for modality, sigma in desc.items():
                if modality == 'weight':
                    continue
                if modality == 'xy':
                    feature = ERSR._get_mesh(N, height_pred, width_pred, device)
                else:
            ...
            kernel = weight * ERSR._create_kernels_from_features(features, kernels_radius)
    ...
    def _create_kernels_from_features(features, radius):
        ...
        kernels = ERSR._unfold(features, radius)
