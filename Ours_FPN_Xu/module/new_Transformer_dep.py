    ...
    def forward(self, x):
        ...
        for k in range(3):
            ...
            for i in range(len(x_up)):
                la_k_1 = self.la_key_1(x_up[i])
                la_k_2 = self.la_key_2(x_up[i])
            ...
            value_2 = []
            for i in range(len(x_up)):
                la_v_1 = self.la_value_1(x_up[i])
                la_v_2 = self.la_value_2(x_up[i])
            ...
            # query_2 = self.la_query_2(la_res).view(b, 1, c//3, -1)
            # query_2 = query_2.permute(0, 3, 1, 2).contiguous()
            #
            # la_att = torch.matmul(query_2, key) * ((c//3) ** -.5)
            # la_att = F.softmax(la_att, dim=-1)
            #
            ...
            #
            # query_3 = self.la_query_3(la_res).view(b, 1, c//3, -1)
            # query_3 = query_3.permute(0, 3, 1, 2).contiguous()
            #
            # la_att = torch.matmul(query_3, key) * ((c//3) ** -.5)
            # la_att = F.softmax(la_att, dim=-1)
            #
