...
def build_optimizer_(cfg: CfgNode, model: torch.nn.Module) -> torch.optim.Optimizer:
    """
    ...
    params = [
        {
            "params": [
                ...
                if "backbone" in name and "bias" in name
            ],
        ...
        },
        {
            "params": [
                p for name, p in model.named_parameters()
                if "backbone" in name and "bias" not in name
            ],
            "lr": cfg.SOLVER.BASE_LR * 0.1
        },
        {
            "params": [
                p for name, p in model.named_parameters()
                if "backbone" not in name and "bias" in name
            ],
            "weight_decay": 0,
        },
        {
            "params": [
                p for name, p in model.named_parameters()
                if "backbone" not in name and "bias" not in name
            ],
        }
    ]
