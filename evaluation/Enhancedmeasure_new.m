function [score]= Emeasure(sal,GT, Thresholds)
...
%Special case:
% if the GT is completely black
AllBlack = (sum(dGT(:))==0);

%if the GT is completely white
AllWhite = (sum(~dGT(:))==0);
[w,h] = size(GT);
area = w*h;
for i=1:NT
    ...
    %Special case:
    if AllBlack% if the GT is completely black
        enhanced_matrix = 1.0 - dFM; %only calculate the black area of intersection
    elseif AllWhite%if the GT is completely white
    ...
    %3.Emeasure score
    score(i) = sum(enhanced_matrix(:))./(area - 1 + eps);
 end
end
