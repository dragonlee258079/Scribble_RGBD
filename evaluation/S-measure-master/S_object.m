...
prediction_fg(~GT)=0;
%prediction_fg(~GT)=0 <=> prediction_fg = prediction_fg.*GT;
O_FG = Object(prediction_fg,GT);

% compute the similarity of the background
prediction_bg = 1.0 - prediction;
prediction_bg(GT) = 0;
%prediction_bg(GT)=0 <=> prediction_bg = prediction_bg.*(1-GT);
...
% x = mean2(prediction(GT)) equals the following:
% % test = sum(sum(prediction.*GT));
% % a = sum(sum(GT));
% % x = test/a;
